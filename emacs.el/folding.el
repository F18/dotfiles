...
;;              Anders Lindgren <andersl@csd.uu.se>
;; Created:     1992
;; RCS version: $Revision: 2.54 $
;; Date:        $Date: 1999/01/07 12:24:47 $
;; Keywords:    tools
...
;;            (define-key  folding-mode-map [mouse-3]   'fold-hide-current-entry)
...
;; - Byte compiler error fix: (fold-bind-outline-compatible-keys):
;;   'fold-show-all lacked the quote.
;;
...
;; - folding.el (fold-mouse-yank-at-point): Renamed from
;;   `fold-mouse-operate-at-point'. The name is similar to Emacs standard
...
;;   folding mimic them
;;   (fold-show-subtree): Renamed to `fold-show-current-subtree'
;;   according to allout.el
;;   (fold-hide-subtree): Renamed to `fold-hide-current-subtree'
;;   according to allout.el
;;   (fold-enter): Renamed to `fold-shift-in'
;;   according to allout.el
;;   (fold-exit): Renamed to `fold-shift-out'
;;   according to allout.el
;;   (fold-move-up): Renamed to `fold-previous-visible-heading'
;;   according to allout.el
;;   (fold-move): Renamed to `fold-next-visible-heading'
;;   according to allout.el
;;   (fold-top-level): Renamed to `fold-show-all'
;;   according to allout.el
;;   (fold-show): Renamed to `fold-show-current-entry'
;;   according to allout.el
;;   (fold-hide): Renamed to `fold-hide-current-entry'
;;   according to allout.el
;;   (fold-region-open-close): While loop rewritten so that if user
;;   is already on a fold mark, then close current fold. This also
;;   fixed the show/hide subtree problem.
;;   (fold-hide-current-subtree): If use hide subtree that only had one
;;   fold, then calling this function caused error. The reason was error
;;   in `fold-pick-move'
;;   (fold-pick-move): Test that `moved' variable is integer and only then
;;   move point. This is the status indicator from `fold-find-fold-mark'
;;   (fold-find-fold-mark): Fixed. mistakenly moved point when checking
...
;; - `fold-get-mode-marks' is now defsubst.
;;
...
;;   cause failing of fold-convert-buffer-for-printing in the following situation
...
;;   }}} Main topic
;;
;;   And when he did fold-convert-buffer-for-printing, it couldn't remove those closing
...
;; - Corrected function fold-show-current-subtree, which didn't find the
...
;;   fold-show-current-subtree          :+ new user cmd "C-s", hides too
;;
...
;;   fold-mouse-yank-at-point:! v, added 'up 'down
...
;;   current 'point'.
;;   fold-mouse-yank-at-point: controls it. Phwew, I like this one a lot!
...
;;   fold-next-visible-heading  :+ f, from tinyfold.el
;;   fold-previous-visible-heading :+ f, from tinyfold.el
...
(defvar fold-version  (substring "$Revision: 2.54 $" 11 15)
...
(defmacro fold-kbd (key function)
  "folding: define key macro.
This is used when assigning keybindings to `folding-mode-map' using
  ...
  (cond
   (fold-xemacs-p
    (define-key folding-mode-map '(shift button2)
      'fold-mouse-context-sensitive)
    ;; (define-key folding-mode-map '(double button3) 'fold-hide-current-entry)
   ...
   (t
    ...
    (define-key folding-mode-map [double-mouse-3] 'fold-hide-current-entry)
...
`fold-bind-backward-compatible-keys' instead."
  ...
  (fold-kbd ">"     'fold-shift-in)
  (fold-kbd "<"	    'fold-shift-out)
  (fold-kbd "\C-t"  'fold-show-all)
  (fold-kbd "\C-f"  'fold-fold-region)
  (fold-kbd "\C-s"  'fold-show-current-entry)
  (fold-kbd "\C-x"  'fold-hide-current-entry)
  (fold-kbd "\C-o"  'fold-open-buffer)
  (fold-kbd "\C-w"  'fold-whole-buffer)

  (fold-kbd "\C-r" 'fold-convert-buffer-for-printing)


  (fold-kbd  "\C-v" 'fold-pick-move)
  (fold-kbd  "v"    'fold-previous-visible-heading)
  (fold-kbd  " "    'fold-next-visible-heading)

  ;;  C-u:  kinda "up" -- "down"

  (fold-kbd "\C-u"  'fold-toggle-enter-exit)
  (fold-kbd "\C-q"  'fold-toggle-show-hide)

  ;; Think "#" as a 'fence'

  (fold-kbd "#"	    'fold-region-open-close)

  ;; Esc-; is the standard emacs commend add key.

  (fold-kbd ";"	    'fold-comment-fold)
  (fold-kbd "%"	    'fold-convert-to-major-folds)

  (fold-kbd "\C-y"  'fold-show-current-subtree)
  (fold-kbd "\C-z"  'fold-hide-current-subtree)
  (fold-kbd "\C-n"  'fold-display-name)
  )
...
`fold-bind-backward-compatible-keys' instead."
  ...
  (fold-kbd ">"	    'fold-shift-in)
  (fold-kbd "<"	    'fold-shift-out)
  (fold-kbd "\C-n"  'fold-next-visible-heading)
  (fold-kbd "\C-p"  'fold-previous-visible-heading)

  ;; ("\C-u" outline-up-current-level)
  ;; ("\C-f" outline-forward-current-level)
  ;; ("\C-b" outline-backward-current-level)
  ;;  (fold-kbd "\C-i"  'fold-show-current-subtree)

  (fold-kbd "\C-s" 'fold-show-current-subtree)
  (fold-kbd "\C-h" 'fold-hide-current-subtree)

  (fold-kbd "!"     'fold-show-all)

  (fold-kbd "\C-d"  'fold-hide-current-entry)
  (fold-kbd "\C-o"  'fold-show-current-entry)
  ...
  (fold-kbd "\C-a"  'fold-open-buffer)
  (fold-kbd "\C-q"  'fold-whole-buffer)

  (fold-kbd "\C-r"  'fold-convert-buffer-for-printing)
  (fold-kbd "\C-w"  'fold-fold-region)
...
(defcustom fold-shift-in-on-goto t
...
(when fold-advice-instantiate
  ...
  (defadvice goto-line (around fold-goto-line first activate)
    "Go to line ARG, entering folds if `fold-shift-in-on-goto' is t.
		...
		(progn
		  ...
		  (let ((goal (point)))
		    (while (prog2 (beginning-of-line)
			       (if (eq fold-shift-in-on-goto 'show)
				   (progn
				     (fold-show-current-entry t t)
...
the default is C - c @."
  (interactive)
  (fold-kbd "\C-z" 'fold-shift-in)
  (fold-kbd "\C-x" 'fold-shift-out))
...
;;    ;; (fold-kbd "d" 'hide-ifdef-define)
;;    ;; (fold-kbd "u" 'hide-ifdef-undef)
;;    ;; (fold-kbd "D" 'hide-ifdef-set-define-alist)
;;    ;; (fold-kbd "U" 'hide-ifdef-use-define-alist)
;;
;;    ;; (fold-kbd "h") 'hide-ifdefs)
;;    ;; (fold-kbd "s") 'show-ifdefs)
;;    ;; (fold-kbd "\C-d") 'hide-ifdef-block)
;;    ;; (fold-kbd "\C-s") 'show-ifdef-block)
;;
;;    ;; (fold-kbd "\C-q" 'hide-ifdef-toggle-read-only)
;;    )
...
(defvar fold-mouse-yank-at-point t
...
(defcustom fold-behave-table
  '((close      fold-hide-current-entry)
    (open       fold-show-current-entry)              ; Could also be `fold-shift-in'.
    (up         fold-shift-out)
    (other      fold-mouse-call-original)
    )
  "*Table of of logical commands and their associated functions.
If you want fold to behave like `fold-shift-in', when it 'open' a fold, you just
...
Use function  `fold-add-to-marks-list' to add more fold marks. The function
...
(defun fold-easy-menu-define ()
  ...
  (easy-menu-define
   ...
   (list
    ...
    ["Enter Fold"			fold-shift-in		t]
    ["Exit Fold"			fold-shift-out		t]
    ["Show Fold"			fold-show-current-entry		t]
    ["Hide Fold"			fold-hide-current-entry		t]
    "----"
    ["Show Whole Buffer"		fold-open-buffer		t]
    ["Fold Whole Buffer"		fold-whole-buffer		t]
    ["Show subtree"			fold-show-current-subtree	t]
    ["Hide subtree"			fold-hide-current-subtree	t]
    ["Display fold name"		fold-display-name		t]
    "----"
    ["Move previous"			fold-previous-visible-heading	t]
    ["Move next"			fold-next-visible-heading	t]
    ["Pick fold"			fold-pick-move			t]
    "----"
    ["Foldify Region"			fold-fold-region	t]
    ["Open or close folds in region"	fold-region-open-close	t]
    ["Open folds to top level"		fold-show-all		t]
    "----"
    ["Comment text in fold"		fold-comment-fold	t]
    ["Convert for printing(temp buffer)" fold-convert-buffer-for-printing t]
...
(defun fold-install  ()
  ...
  (or (assq 'folding-mode minor-mode-alist)
      (push '(folding-mode fold-mode-string) minor-mode-alist))
...
;;; ----------------------------------------------------------------------
;;;
(defsubst fold-get-mode-marks (&optional mode)
...
Return:
  ...
  (let* (elt
    ...
    (unless (setq elt (assq (or mode major-mode) fold-mode-marks-alist))
...
(defun fold-region-has-fold-marks-p (beg end)
  ...
  (save-excursion
    (goto-char beg)
    (when (memq (fold-mark-look-at) '(1 11))
      (goto-char end)
...
(defun fold-region-open-close (beg end &optional close)
  ...
  (let* ((func (if (null close)
		   'fold-show-current-entry
		 'fold-hide-current-entry))
         tmp
         )
    (save-excursion
      ...
      (while (and
	      ;;   the fold-show-current-entry/hide will move point
		...
		(end-of-line)
		t)
	      (fold-next-visible-heading)
	      (< (point) end)))
        )))

;;; ----------------------------------------------------------------------
;;;
(defun fold-hide-current-subtree  ()
  "Call `fold-show-current-subtree' with argument 'hide."
  (interactive)
  (fold-show-current-subtree 'hide))

;;; ----------------------------------------------------------------------
;;;
(defun fold-show-current-subtree (&optional hide)
...
Please see the variable `fold-mouse-yank-at-point'."
  'fold-mouse-yank-at-point)
...
fold-convert-buffer-for-printing:  `\\[fold-convert-buffer-for-printing]'
...
Fold marks
    ...
    and type `\\[fold-shift-in]'.  You should no longer be able to see the rest
    of the file, just the contents of the fold, which you couldn't see
    before.  You can use `\\[fold-shift-out]' to leave a fold, and you can enter
...
Managing folds
    ...
    `fold-shift-out' will attempt to tidy the current fold just before exiting
...
Mouse behavior
    ...
    variable `fold-mouse-yank-at-p'.

    To customise the mouse actions, look at `fold-behave-table'.
"
  (interactive)
...
;;; ----------------------------------------------------------------------
;;;
(defun fold-previous-visible-heading ()
  "Move upward fold headers."
  (interactive)
  (beginning-of-line)
  (fold-next-visible-heading 'backward))
...
;;; ----------------------------------------------------------------------
;;;
(defun fold-shift-in (&optional noerror)
  ...
  (let ((goal (point)))
    (if (fold-skip-ellipsis-backward)
        (while (prog2 (beginning-of-line)
                      (fold-shift-in t)
                      (goto-char goal)))
      (let ((data (fold-show-current-entry noerror t)))
        (and data
             (progn
               (setq fold-stack
                     (if fold-stack
...
(defun fold-shift-out (&optional event)
...
(defun fold-show-current-entry (&optional event noerror noskip)
...
(defun fold-toggle-enter-exit ()
  "Run fold-shift-in or fold-shift-out depending on current line's contents."
  (interactive)
  (beginning-of-line)
...
(defun fold-toggle-show-hide ()
  "Run fold-show-current-entry or fold-hide-current-entry depending on current line's contents."
  (interactive)
  (beginning-of-line)
  (let ((current-line-mark (fold-mark-look-at)))
    (if (and (numberp current-line-mark)
...
(defun fold-hide-current-entry (&optional event)
  "Close the fold around the point, undo effect of `fold-show-current-entry'."
  (interactive)
  (fold-skip-ellipsis-backward)
  (let (start end)
...
;;}}}
;;{{{ fold-convert-buffer-for-printing

(defun fold-convert-buffer-for-printing (&optional buffer pre-title post-title pad)
...
(defun fold-goto-char (pos)
  ...
  (if (eq pos (point))                  ; Point inside narrowed area?
      nil
    (fold-show-all)                    ; Fold everything and goto top.
    (goto-char pos))
  ;; Enter if point is folded.
  (if (fold-point-folded-p pos)
      (progn
        (fold-shift-in)                    ; fold-shift-in can change the pos.
        (setq fold-isearch-stack fold-stack)
...
Folding: function usage error, mode with `comment-end' is not supported.")))
    ...
    (when (or (and uncomment state)
      ...
      (when closed (save-excursion (fold-show-current-entry)))
