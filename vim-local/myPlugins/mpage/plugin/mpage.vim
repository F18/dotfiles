...
"   Date:   Jan 16, 2013
"   Version: 1s	ASTRO-ONLY
" ---------------------------------------------------------------------
"  Load Once: {{{1
if &cp || exists("g:loaded_mpage")
 finish
endif
let g:loaded_mpage= "v1s"
let s:keepcpo      = &cpo
set cpo&vim
"DechoRemOn
...
"  call Dfunc("s:RestoreUserSettings()")
  if exists("s:keepfen".bufnr("%"))|let &fen = s:keepfen|unlet s:keepfen{bufnr("%")}|endif  " foldenable
  if exists("s:keepscb".bufnr("%"))|let &scb = s:keepscb|unlet s:keepscb{bufnr("%")}|endif  " scrollbind
...
"   call Decho("installing maps for buffer<".bufname("%").">")
   ...
   "noremap <buffer> <c-b>		z^
   noremap <buffer> <c-e>		<c-e>:call <SID>ReSync()<cr>
   "noremap <buffer> <c-f>		z+
   "noremap <buffer> <c-l>		<c-l>:call <SID>ReSync()<cr>
   noremap <buffer> <c-y>		<c-y>:call <SID>ReSync()<cr>
   noremap <buffer> z<cr>		z<cr>:call <SID>ReSync()<cr>
   noremap <buffer> zb			zb:call <SID>ReSync()<cr>
   noremap <buffer> zt			zt:call <SID>ReSync()<cr>
   noremap <buffer> z-			z-:call <SID>ReSync()<cr>
   noremap <buffer> z.			z.:call <SID>ReSync()<cr>
   noremap <buffer> zz			zz:call <SID>ReSync()<cr>
   noremap <buffer> <PageDown>	:call <SID>MpageDown()<cr>
   noremap <buffer> <PageUp>	:call <SID>MpageUp()<cr>
  endif
